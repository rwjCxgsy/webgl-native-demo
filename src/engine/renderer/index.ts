...
import { CustomBoxGeometry, CustomGeometry } from '../geometry/customGeometry';
...
class Renderer {
  ...
  shadowCamera = new Camera(90, 1, 10, 200);

  shadowProgram?: WebGLProgram;

  defaultTexture?: WebGLTexture;
  frustumEntity: Entity = new Entity(
    new CustomBoxGeometry(),
    new LineMaterial()
  );
  ...
  transUniforms(
    ...
    lights.forEach((light) => {
      switch (light.type) {
        case 'PointLight':
          const u_diffuseColor = gl.getUniformLocation(
            programInfo.program,
            'u_diffuseColor'
          );
          ...
          u_diffuseColor && gl.uniform3fv(u_diffuseColor, light.color);
        ...
        case 'AmbientLight':
          ...
          u_ambientColor && gl.uniform3fv(u_ambientColor, light.color);
          break;
        default:
          break;
      }
  ...
  createAttributes(geometry: Geometry): {
    ...
    if (geometry.indices) {
      ...
      //@ts-ignore
      result.indicesBuffer = indicesBuffer;
    }
    return result;
  }
  ...
  renderFrustum(camera: Camera) {
    ...
    if (frustumProgramInfo.indicesBuffer) {
