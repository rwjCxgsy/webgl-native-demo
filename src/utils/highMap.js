...
function Normal(nx, ny, nz) {
  ...
  this.compareNormal = function (normal) {
    var DIFF = 0.000001;
    if (
      this.nx - normal.nx < DIFF &&
      this.ny - normal.ny < DIFF &&
      this.nz - normal.nz < DIFF
    ) {
      return false;
    } else {
      return true;
...
function caleNormalVector(yArray) {
  ...
  for (var i = 0; i < rows; i++) {
    for (var j = 0; j < cols; j++) {
      ...
      var vNormal1 = vectorNormal(
        getCrossProduct([vxa, vya, vza], [vxb, vyb, vzb])
      );
      ...
      var vNormal2 = vectorNormal(
        getCrossProduct([vxb, vyb, vzb], [vxa, vya, vza])
      );
  ...
  mat4.rotateX(m4, mat4.create(), -Math.PI / 2);

  normals.flat(1).forEach((v) => {
    const v3 = vec3.create();
    v3[0] = v[0];
    v3[1] = v[1];
    v3[2] = v[2];
    const out = vec3.create();
    vec3.transformMat4(out, v3, m4);
...
function getNormalByHighMap(image, width, height) {
  ...
  function getgray() {
    ...
    for (var i = 0; i < imageData.data.length; i += 4) {
      ...
      var h = (r + g + b) / 3;
      const y = (h / 255) * LAND_HIGHEST + LAND_HIGH_ADJUST;
      result1[j] = y;
      heightY.push(y);
      j++;
    }
  ...
  const normals = caleNormalVector(result);
...
export { caleNormalVector, getNormalByHighMap };
